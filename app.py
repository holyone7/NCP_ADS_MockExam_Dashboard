...
with tab_exam:
    ...
    if submitted:
        ...
        if st.button("📄 Generate Professional Review PDF"):
            ...
            y = height-110
            c.setFont("Helvetica-Bold", 12); c.drawString(72, y, "DOMAIN PERFORMANCE SUMMARY")
            y -= 16; c.setFont("Helvetica", 10)
            total_q = len(filtered); overall_acc = (total_correct/total_q)*100 if total_q else 0.0
            for d,(s,t) in domain_scores.items():
                c.drawString(72, y, f"{d}"); c.drawRightString(width-180, y, f"{s} / {t}")
                c.drawRightString(width-72, y, f"({(s/t)*100:.1f}%)"); y -= 12
                if y < 96: c.showPage(); y = height-72; c.setFont("Helvetica", 10)
            y -= 6; c.setFont("Helvetica-Bold", 10); c.drawString(72, y, f"Overall Accuracy: {overall_acc:.1f}%"); y -= 18

            c.setFont("Helvetica-Bold", 12); c.drawString(72, y, "WRONG ANSWER DETAILS"); y -= 16; c.setFont("Helvetica", 10)
            if not wrongs:
                c.drawString(72, y, "Great job! No wrong answers in this session."); y -= 12
            else:
                ...
                for r in wrongs:
                    ...
                    for line in lines:
                        for w in _tw.wrap(line, width=95):
                            c.drawString(72, y, w); y -= 12
                            if y < 72: c.showPage(); y = height-72; c.setFont("Helvetica", 10)
                    if y < 84: c.showPage(); y = height-72; c.setFont("Helvetica", 10)
            c.setFont("Helvetica", 8); c.drawCentredString(width/2, 40, "Generated by NCP-ADS Mock Exam Dashboard"); c.save()
